      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16 mt-5">
        {problems.map((problem, index) => (
          <motion.div
            ...
            } p-6 rounded-xl text-center font-semibold shadow-xl py-8 hover:bg-gray-200 group`}
            initial={{ opacity: 0, y: 50 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.6, delay: index * 0.1 }}
            whileHover={{ scale: 1.05 }}
          >
            <div className="flex justify-center items-center group-hover:bg-gray-300 w-fit p-4 rounded-full mx-auto">
                {React.cloneElement(problem.icon, { size: 30 })}
            </div>
            <p className="mt-3 text-lg">{problem.text}</p>
          </motion.div>
        ))}
      </div>
    </section>
